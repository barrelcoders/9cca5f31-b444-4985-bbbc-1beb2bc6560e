...
class EntriesController extends Controller {
	...
	public function actionGetEntriesDetail($arrivalDate){
		$entriesData = Entries::model()->findAllByAttributes(array(
			'arrival_date' => date('Y-m-d', strtotime($arrivalDate)),
			));
		
		
		$count = Entries::model()->count('arrival_date="'.date('Y-m-d', strtotime($arrivalDate)).'"');
		if($count > 0 ){
			$content = "We have found following entries with same Details:<br/><table><tr><th>Arrival Date</th><th>Client Name</th><th>Agency Name</th></tr>";
			foreach($entriesData as $e){
				$content .="<tr><td>".date('d-m-Y', strtotime($e->arrival_date))."</td>";
				$content .="<td>".$e->client_name."</td>";
				$content .="<td>".AgencyMaster::model()->findByPK($e->agency_id_fk)->name."</td>";
				$content .="</tr>";
			}
			$content .="</table>"; 
		}
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Entries'])) {
            ...
            if ($model->save()) {
                ...
                $this->redirect(array('Arrival/create', 'entry' => $model->id));
            } else
