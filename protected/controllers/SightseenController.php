...
class SightseenController extends Controller {
	...
	public function getNotAvailableServiceId(){
		$service_id = 0;
		$entry_id = Arrival::model()->findByPK($id)->entry_id_fk;
		$agency_id = Entries::model()->findByPK($entry_id)->agency_id_fk;
		if(AgencyMaster::model()->findByPK($agency_id)->shops){
			$shopArray = explode(",", AgencyMaster::model()->findByPK($agency_id)->shops);
                        $shop_id = $shopArray[0];
			$branch_id = ApprovedShops::model()->findByPK($shop_id)->branch_id_fk;
			if(ServiceMaster::model()->exists(
				'service_name=:service_name AND branch_id_fk=:branch_id_fk',
				array(':service_name'=>'NA', ':branch_id_fk'=>$branch_id))){
				$service_id = ServiceMaster::model()->find("service_name='NA'and branch_id_fk='" . $branch_id . "'")->id;
			}
			else{
				$model=new ServiceMaster;
				$model->branch_id_fk = $branch_id;
				$model->service_name = 'NA';
				$model->short_code = 'NA';
				$model->save();
				$service_id = $model->id;
			}
        ...
        if (isset($_POST['SiteseenServices'])) {
			...
			if($model->save()){
				...
				for($i=1; $i<=$total; $i++){
					...
					$ServicesShops = implode(",", $_POST['SiteseenServices']['shops'][$i-1]);
					$SiteseenServices->shops = $ServicesShops;
					if(isset($_POST['SiteseenServices']['services'][$i-1])){
						$SiteseenServices->services = implode(",", $_POST['SiteseenServices']['services'][$i-1]);
					}
					else{
						$SiteseenServices->services = $this->getNotAvailableServiceId($arrival);
					}
					...
					$SiteseenServices->additional_service = $_POST['SiteseenServices']['additional_service'][$i-1][0];
					if($SiteseenServices->save(false)){
						...
						for($j=1; $j<=$total_guides; $j++){
							...
							$SitescreenServiceGuides->save(false);
						...
						for($k=1; $k<=$total_vehicles; $k++){
							for($l=1; $l<=intVal($_POST['SiteseenServices']['vehicles']['totalVehicle'][$i-1][$k-1]); $l++){
								if(isset($_POST['SiteseenServices']['vehicles']['vehicle_id_fk'][$i-1][$k-1])){
									$SitescreenServiceVehicles = new SiteseenServiceVehicles;
									$SitescreenServiceVehicles->vehicle_id_fk = $_POST['SiteseenServices']['vehicles']['vehicle_id_fk'][$i-1][$k-1];
									...
									$SitescreenServiceVehicles->nameOfVehicle = VehicleMaster::model()->findByPK($_POST['SiteseenServices']['vehicles']['vehicle_id_fk'][$i-1][$k-1])->name;
									$SitescreenServiceVehicles->save(false);
									$SitescreenServiceVehicles = null;
								}
							}
						}
                ...
                $this->redirect(array('departure/create', 'arrival' => $this->arrival_id));
			}
			else{
			...
			//echo "<pre>";print_r($_POST['SiteseenServices']);echo "<pre>";exit;
				...
				for($i=1; $i<=$total; $i++){
					...
					if(isset($_POST['SiteseenServices']['services'][$i-1])){
						...
						$SiteseenServices->services = $this->getNotAvailableServiceId(Sightseen::model()->findByPK($id)->arrival_id_fk);
					}
					$SiteseenServices->entrance_by = $_POST['SiteseenServices']['entrance_by'][$i-1];
					$SiteseenServices->reporting_place = $_POST['SiteseenServices']['reporting_place'][$i-1];
					$SiteseenServices->total_guide = $_POST['SiteseenServices']['Total_Guide'][$i-1];
					...
					if(intVal($data->sheets[0]['numRows']) === count($data->sheets[0]['cells'])){
						for ($j = 2; $j <= $data->sheets[0]['numRows']; $j++){
							...
							if($total_vehicle == $serviceVehicleCount){
								...
								if(VehicleMaster::model()->exists('name=:name',array(':name'=>$name))){
										$uploadModel=new SiteseenServiceVehicles;
										$uploadModel->siteseen_service_id_fk = $id;
										$uploadModel->vehicle_id_fk = VehicleMaster::model()->findByAttributes(array('name' => $name))->id;
										$uploadModel->acOrNot = $acOrNot;
										$uploadModel->nameOfVehicle = $name;
										$uploadModel->totalVehicle = 1;
										$uploadModel->save(false);
										$uploadModel=null;
										array_push($this->importStatus,array("<b>".$name."</b> Vehicle for Siteseen Service successfully saved ","success"));
								}
								else{
