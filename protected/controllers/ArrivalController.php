...
class ArrivalController extends Controller {
        ...
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('index', 'view', 'create', 'update', 'delete', 'TrainNumberArrivalTime',
                    ...
                    'ValueUpdate', 'ArrivalVehicleValueUpdate', 'upload', 'UploadVehicle', 'admin', 'delete'),
                'users' => array('*'),
            ),
    ...
    public function actionCreate($entry) {
        ...
        if (isset($_POST['Arrival'])) {
			...
			if(($_POST['Arrival']['arrival_by']=='Train' || $_POST['Arrival']['arrival_by']=='Bus' || $_POST['Arrival']['arrival_by']=='Flight') && $_POST['Arrival']['vehicle_required']=='Yes'){
				$model->transportOrSurface = 'Transport';
            }
			else if(($_POST['Arrival']['arrival_by']=='Train' || $_POST['Arrival']['arrival_by']=='Bus' || $_POST['Arrival']['arrival_by']=='Flight') && $_POST['Arrival']['vehicle_required']=='No'){
				$model->transportOrSurface = 'Transport';
            }
			else if($_POST['Arrival']['arrival_by']=='Surface' && $_POST['Arrival']['vehicle_required']=='Yes'){
				$model->transportOrSurface = 'Transport';
            }
			else if($_POST['Arrival']['arrival_by']=='Surface' && $_POST['Arrival']['vehicle_required']=='No'){
				$model->transportOrSurface = 'Surface';
            }
            ...
            if ($model->save(false)) {
                if ($_POST['ArrivalVehicle']) {
                    ...
                    for ($i = 1; $i < sizeof($_POST['ArrivalVehicle']['vehicle_id_fk']); $i++) {
                        for ($j = 1; $j <= intVal($_POST['ArrivalVehicle']['noOfVehicle'][$i]); $j++) {
                            ...
                            $modelArrivalVehicle->vehicle_id_fk = $_POST['ArrivalVehicle']['vehicle_id_fk'][$i];
                            $modelArrivalVehicle->category_id_fk = VehicleMaster::model()->findByPK($_POST['ArrivalVehicle']['vehicle_id_fk'][$i])->category_id_fk;
                            $modelArrivalVehicle->arrival_id_fk = $model->id;
							$modelArrivalVehicle->nameOfVehicle = VehicleMaster::model()->findByPK($_POST['ArrivalVehicle']['vehicle_id_fk'][$i])->name;
							$modelArrivalVehicle->save(false);
                        }
                    }
                }
				
                $this->redirect(array('sightseen/create', 'arrival' => $model->id));
            } else {
    ...
    public function actionTrainNumberArrivalTime() {
        ...
        if ($_POST['val'] == "Other") {
            ...
            echo $model->number . "," . $model->arrival_time .", ".Places::model()->findByPK($model->from)->name;
            exit;
        }
    }
    ...
    public function actionBusNumberArrivalTime() {
        ...
        if ($_POST['val'] == "Other") {
            ...
            echo $model->arrival_time.", ".Places::model()->findByPK($model->from)->name;
            exit;
        }
    }
