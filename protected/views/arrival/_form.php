    ...
    <div class="box gradient">
        ...
        <div class="content top ">
            ...
            <label class="control-label span3" for="Phone_no">Client Name</label>
				<div class="controls span7">
					<input type="text" class="row-fluid" readonly="true" value="<?php echo Entries::model()->findByPK($this->entry_id)->client_name; ?>"/>
				</div>
			</div>
			<div class="form-row control-group row-fluid">
            ...
            <div class="form-row control-group row-fluid">
                <label class="control-label span3">Arrival By</label>
                <div class="controls span7">
                    <?php
            ...
            <div id="arrivalByTrain" class="row-fluid" <?php echo ($model->arrival_by == "Train") ? "style='display:block;'" : "style='display:none;'"; ?>>
				...
				<div class="form-row control-group row-fluid">
                    <label class="control-label span3" for="Phone_no">From</label>
                    <div class="controls span7">
                        <input type="text" name="Arrival[train_arrival_from]" readonly="readonly" id="txtTrainFrom"
                               value="<?php if ($model->train_id_fk != 0) echo Places::model()->findByPK(TrainMaster::model()->findByPK($model->train_id_fk)->from)->name; ?>" />
                    </div>
                </div>
            ...
            <div id="arrivalByBus" class="row-fluid" <?php echo ($model->arrival_by == "Bus") ? "style='display:block;'" : "style='display:none;'"; ?>>
                <div class="form-row control-group row-fluid" id="A_train_name">
					...
					echo CHtml::activeDropDownList($model, 'bus_id_fk', CHtml::listData(BusMaster::model()->findAll(), 'id', 'name'), array(
						'ajax' => array(
							...
							'success' => 'function(data){																								
									var arr = data.split(",");
									$("#txtBusArrivalTime").val(arr[0]);
									$("#txtBusArrivalFrom").val(arr[1]);
								}'
						),
						'id' => 'slBus',
						'class' => 'other-select',
                ...
                <div class="form-row control-group row-fluid">
                    <label class="control-label span3" for="Phone_no">From</label>
                    <div class="controls span7">
                        <input type="text" readonly="readonly" name="Arrival[bus_from]" id="txtBusArrivalFrom"
                               value="<?php if ($model->bus_id_fk != 0) echo Places::model()->findByPK(BusMaster::model()->findByPK($model->bus_id_fk)->from)->name; ?>" />
                    </div>
                </div>
            </div>
            ...
            <div id="arrivalByFlight" class="row-fluid" <?php echo ($model->arrival_by == "Flight") ? "style='display:block;'" : "style='display:none;'"; ?>>
                    ...
                    <div class="controls span7">
						...
						echo CHtml::activeDropDownList($model, 'flight_id_fk', CHtml::listData(FlightMaster::model()->findAll(), 'id', 'name'), array(
							'ajax' => array(
								...
								'success' => 'function(data){																						
										var arr = data.split(",");
										$("#txtFlightArrivalTime").val(arr[0]);
										$("#txtFlightArrivalFrom").val(arr[1]);
									}'
							),
							'id' => 'slFlight',
							'class' => 'other-select',
                ...
                <div class="form-row control-group row-fluid">
                    <label class="control-label span3" for="Phone_no">From</label>
                    <div class="controls span7">
                        <input type="text" readonly="readonly" name="Arrival[flight_arrival_from]" id="txtFlightArrivalFrom"
                               value="<?php if ($model->flight_id_fk != 0) echo Places::model()->findByPK(FlightMaster::model()->findByPK($model->flight_id_fk)->from)->name; ?>"/>
                    </div>
                </div>
            ...
            <div id="arrivalBySurface" <?php echo ($model->arrival_by == "Surface") ? "style='display:block;'" : "style='display:none;'"; ?>>
                ...
                <div class="form-row control-group row-fluid" id="A_from" >
                    ...
                    <div class="controls span7">
					<?php
					echo $form->dropDownList($model, 'from', CHtml::listData(Places::model()->findAll(), 'id', 'name'), 
                        ...
                        <tr>
                            <td>
                                <div class="form-row control-group row-fluid">
                                    <div class="controls span7">
										<?php
											echo $form->dropDownList($model_arrivalVehicle, 'vehicle_id_fk[]', CHtml::listData(VehicleMaster::model()->findAll(), 'id', 'name'), array('empty' => 'Select Vehicle Name'));
										?> 
                                    </div>
                                </div>
                            </td>
...
?> 
                ...
                <span id="arrivalVehicleDoc">
                                        ...
                                        if (Yii::app()->controller->action->id == 'update' && $model->total_vehicle > 0) {
                                            ...
                                            $UniquCategoryData = Yii::app()->db->createCommand("SELECT DISTINCT(vehicle_id_fk) FROM arrivalvehicle WHERE arrival_id_fk=" . $_GET['id'] . ";")->queryAll();
                                            foreach ($UniquCategoryData as $d) {
                                               $countVehicle = count(ArrivalVehicle::model()->findAll("arrival_id_fk = " . $_GET['id'] . " AND vehicle_id_fk=" . $d['vehicle_id_fk']));
                                                $acOrNot = Yii::app()->db->createCommand("SELECT acOrNot FROM arrivalvehicle WHERE arrival_id_fk = " . $_GET['id'] . " AND vehicle_id_fk=" . $d['vehicle_id_fk'] . " LIMIT 1")->queryAll();
                                ...
                                <tr>
                                    <td>
                                        <div class="form-row control-group row-fluid">
											...
											echo $form->dropDownList($model_arrivalVehicle, 'vehicle_id_fk[]', CHtml::listData(VehicleMaster::model()->findAll(), 'id', 'name'), array('empty' => 'Select Vehicle Name',
												'options' => array(
													$d['vehicle_id_fk'] => array('selected' => 'true')
												))
											);
											?> 
                                            </div>
